// VERAS COMPREI/PGFN — Templates Satori V1.5.6
...
function inferApto(input) {
  const txt = `${input.titulo || ''} ${input.descricao || ''} ${input.complemento || ''} ${input.complementoEndereco || ''}`;
  const patterns = [
    /(?:apartamento|apto|ap\.?|unidade)\s*(?:de\s*)?(?:n[ºo.]?\s*)?(\d{1,5})/i,
    /(?:^|[,\s])ap\.?\s*(\d{1,5})(?:\b|,)/i,
  ];
...
function normalizeStreetType(tipo) {
  const t = clean(tipo, '').replace(/\.$/, '').toLowerCase();
  if (t === 'r') return 'Rua';
  if (t === 'av') return 'Avenida';
  if (t === 'pc' || t === 'praça') return 'Praça';
  return tipo.charAt(0).toUpperCase() + tipo.slice(1);
}

function extractEndereco(input) {
  const preferred = clean(input.endereco_publico || input.endereco_comercial || input.endereco_formatado || '', '');
  const apto = clean(input.apartamento || input.apto || input.unidade || inferApto(input), '');
  if (preferred) {
    return { linha: limit(preferred, 86), logradouro: preferred, numero: clean(input.numero || input.numeroEndereco, ''), complemento: apto ? `ap. ${apto}` : clean(input.complemento || input.complementoEndereco, '') };
  }

  const all = `${input.titulo || ''} ${input.descricao || ''} ${input.logradouro || ''} ${input.endereco || ''}`;
  const candidatos = [
    /(Rua|R\.?|Avenida|Av\.?|Alameda|Travessa|Rodovia|Estrada|Praça|Praca)\s+([^,\n]+?)\s*,\s*(?:n[ºo.]?|número|numero)?\s*(\d{1,6})(?=\s*,|\s|$)/i,
    /(Rua|R\.?|Avenida|Av\.?|Alameda|Travessa|Rodovia|Estrada|Praça|Praca)\s+([^,\n]+?)\s+(\d{1,6})(?=\s*,|\s|$)/i,
  ];

  let tipo = '', nome = '', numero = clean(input.numero || input.numeroEndereco, '');
  for (const re of candidatos) {
    const m = all.match(re);
    if (m) { tipo = normalizeStreetType(m[1]); nome = clean(m[2]); numero = clean(m[3]); break; }
  }

  if (!nome) {
    const raw = clean(input.logradouro || input.endereco || '', '');
    const m = raw.match(/^(Rua|R\.?|Avenida|Av\.?|Alameda|Travessa|Rodovia|Estrada|Praça|Praca)\s+([^,\n-]+)/i);
    if (m) { tipo = normalizeStreetType(m[1]); nome = clean(m[2]); }
  }

  // Proteção: se o único número encontrado for o número do apartamento, não usar como número do prédio.
  if (apto && numero === apto) numero = clean(input.numeroEndereco || input.numero, '');
  if (/^0$|^1$|^sn$|^s\/n$/i.test(numero)) numero = '';

  const logradouro = nome ? `${tipo || 'Rua'} ${nome}` : clean(input.logradouro || input.endereco || 'Endereço a conferir');
  const complemento = apto ? `ap. ${apto}` : clean(input.complementoEndereco || input.complemento || '', '');
  const linhaParts = [logradouro];
  if (numero) linhaParts.push(numero);
  if (complemento && !/número original não informado/i.test(complemento)) linhaParts.push(complemento);
  return { linha: limit(linhaParts.join(', '), 86), logradouro, numero, complemento };
}

function titleMain(input) {
  const providedTitle = clean(input.titulo, '');
  if (providedTitle) return providedTitle.toUpperCase();
...
function addressLine(input) {
  return extractEndereco(input).linha;
}
...
function explicitPhaseFromInput(input) {
  const raw = clean(
    input.fase_atual_comprei ??
    input.fase_comprei ??
    input.fase_atual ??
    input.fase,
    ''
  ...
  const map = {
    negociacao: { idx: 1, label: 'Negociação' },
    proposta_1: { idx: 2, label: 'Em fase de proposta (1)' },
    proposta1: { idx: 2, label: 'Em fase de proposta (1)' },
    proposta_2: { idx: 3, label: 'Em fase de proposta (2)' },
    proposta2: { idx: 3, label: 'Em fase de proposta (2)' },
    compra_em_andamento: { idx: 4, label: 'Compra em andamento' },
    finalizado: { idx: 5, label: 'Bem finalizado' },
  };
  return map[raw] || null;
}
...
function normalizeImovel(input) {
  ...
  const phase = explicitPhaseFromInput(input) ?? (currentPhase(input, dataPrimeiraRaw, dataNegRaw));
  const endereco = extractEndereco(input);
  ...
  const i = {
    ...
    tituloVenda: titleMain(input), tituloCurto: titleShort(input), enderecoCurto: endereco.linha, endereco, tipo,
