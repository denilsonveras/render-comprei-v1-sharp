// VERAS COMPREI/PGFN — Templates Satori V1.5.4
// Cards comerciais para imóveis PGFN/Comprei: imóvel, cronologia/proposta e documentação.
...
function normalizeImovel(input) {
  ...
  const i = {
    ...
    valorVenda: money(valorVenda), valorAvaliacao: money(valorAvaliacao),
...
export function card1(input) {
  ...
  return el({ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', width: '1280px', height: '720px', padding: '54px 70px', backgroundColor: COLORS.navy, backgroundImage: `linear-gradient(135deg, ${COLORS.navy} 0%, ${COLORS.navy2} 100%)`, color: COLORS.white, fontFamily: 'Inter' }, [
    el({ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }, [ brand(true), tag('LEILÃO DE IMÓVEIS - COMPREI/PGFN') ]),
    el({ display: 'flex', flexDirection: 'column', marginTop: 10 }, [
      ...
      tx({ fontSize: 22, marginTop: 12, color: COLORS.muted, opacity: 0.82, lineClamp: 1 }, 'Imóvel de leilão judicial da Procuradoria-Geral da Fazenda Nacional'),
    ]),
    el({ display: 'flex', justifyContent: 'space-between', gap: 18 }, [
      miniLabel('VALOR ATUAL', i.valorVenda, true), miniLabel('AVALIAÇÃO', i.valorAvaliacao, true),
...
export function card2(input) {
  const i = normalizeImovel(input);
  const phase = i.phase;
  const nextLabel = phase.idx === 1 ? `Proposta (1) após ${i.dataFimNegociacao}` : phase.idx === 2 ? `Proposta (2) após ${i.dataPrimeiraFase}` : 'Acompanhar próximas atualizações';
  return el({ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', width: '1280px', height: '720px', padding: '50px 70px', backgroundColor: COLORS.cream, color: COLORS.ink, fontFamily: 'Inter' }, [
    el({ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }, [
      el({ display: 'flex', flexDirection: 'column', width: 850 }, [
        ...
        tx({ fontSize: 21, color: COLORS.inkSoft, marginTop: 10, lineClamp: 1 }, 'Antes de ofertar, confira regras, prazos, matrícula e condições de pagamento.'),
      ]),
    ...
    el({ display: 'flex', gap: 18, alignItems: 'stretch' }, [
      el({ display: 'flex', flexDirection: 'column', backgroundColor: COLORS.white, borderTop: `4px solid ${COLORS.gold}`, padding: '18px 22px', width: 355, height: 112 }, [
        tx({ fontSize: 14, fontWeight: 900, letterSpacing: 2, color: COLORS.goldDark }, 'FASE ATUAL'),
        tx({ fontSize: 24, fontWeight: 900, color: COLORS.ink, marginTop: 10, lineClamp: 2 }, phase.label),
      ]),
      el({ display: 'flex', flexDirection: 'column', backgroundColor: COLORS.white, borderTop: `4px solid ${COLORS.gold}`, padding: '18px 22px', width: 355, height: 112 }, [
        tx({ fontSize: 14, fontWeight: 900, letterSpacing: 2, color: COLORS.goldDark }, 'PRÓXIMA ETAPA PREVISTA'),
        tx({ fontSize: 21, fontWeight: 800, color: COLORS.ink, marginTop: 10, lineClamp: 2 }, nextLabel),
      ]),
      el({ display: 'flex', flexDirection: 'column', backgroundColor: COLORS.white, borderTop: `4px solid ${COLORS.gold}`, padding: '18px 22px', width: 355, height: 112 }, [
        tx({ fontSize: 14, fontWeight: 900, letterSpacing: 2, color: COLORS.goldDark }, 'PAGAMENTO'),
        tx({ fontSize: 19, fontWeight: 800, color: COLORS.ink, marginTop: 10, lineClamp: 2 }, i.pagamentoResumo),
      ]),
    ]),
  ]);
}
...
export function card3(input) {
  ...
  return el({ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', width: '1280px', height: '720px', padding: '54px 70px', backgroundColor: COLORS.green, backgroundImage: `linear-gradient(135deg, ${COLORS.green} 0%, ${COLORS.green2} 100%)`, color: COLORS.white, fontFamily: 'Inter' }, [
    ...
    el({ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid rgba(255,255,255,0.25)', paddingTop: 18 }, [
      tx({ fontSize: 22, color: COLORS.white, opacity: 0.86 }, 'Dados sensíveis devem ser conferidos reservadamente antes da proposta.'),
